...
const validateProject = [
    ...
    body('createdBy').custom(async (createdBy) => {
    ...
    body('team').isArray().withMessage('Team should be an array of user IDs')
        .custom()(async(team) => {
            if (team.length > 0) {
                //verification des doublons
                const uniqueTeamMember = new Set(team);
                if (uniqueTeamMember.size !== team.length) {
                    throw new Error('Team contains duplicate user IDs')
                }

                //verification des IDs utilisateurs valides
            ...
            return true;
        }).withMessage('Valid user IDs are requiered for the team')
];